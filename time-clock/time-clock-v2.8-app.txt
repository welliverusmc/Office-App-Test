...
const elements = {
  ...
  lastWeekButton: document.getElementById("lastWeekButton"),
...
function renderTicket() {
  ...
  if (!ticket) {
    ...
    elements.ticketTotal.textContent = "0h 0m";
    return;
  }
  ...
  elements.ticketTotal.textContent =
    formatHoursMinutes(ticket.totalMilliseconds);
}


function startOfWeek(timestamp = Date.now(), weekOffset = 0) {
...
function roundedTotalMinutes(milliseconds) {
  const totalSeconds = Math.floor(Math.max(0, milliseconds) / 1000);
  const wholeMinutes = Math.floor(totalSeconds / 60);
  const seconds = totalSeconds % 60;
  return wholeMinutes + (seconds > 30 ? 1 : 0);
}
...
function formatHoursMinutes(milliseconds) {
  return formatMinutes(roundedTotalMinutes(milliseconds));
}
...
function renderWeeklyHours() {
  const weekStart = startOfWeek(Date.now(), selectedWeekOffset);
  const weekEnd = weekStart + (7 * 24 * 60 * 60 * 1000);
  const tickets = Array.isArray(state.tickets) ? [...state.tickets] : [];

  if (state.latestTicket) {
    const alreadyIncluded = tickets.some((ticket) =>
      ticket.clockInTime === state.latestTicket.clockInTime &&
      ticket.clockOutTime === state.latestTicket.clockOutTime
    );

    if (!alreadyIncluded) {
      tickets.push(state.latestTicket);
    }
  }

  const shifts = tickets
    .filter((ticket) => {
      const clockInTime = Number(ticket.clockInTime);
      return Number.isFinite(clockInTime) &&
        clockInTime >= weekStart &&
        clockInTime < weekEnd;
    })
    .sort((a, b) => a.clockInTime - b.clockInTime)
    .map((ticket) => ({
      clockInTime: Number(ticket.clockInTime),
      clockOutTime: Number(ticket.clockOutTime),
      totalMilliseconds: Number(ticket.totalMilliseconds) || 0,
      active: false
    }));
  ...
  if (
    ...
    shifts.push({
      clockInTime: state.activeShift.clockInTime,
      clockOutTime: null,
      totalMilliseconds: Date.now() - state.activeShift.clockInTime,
      active: true
    });
  }

  elements.thisWeekButton.classList.toggle("active", selectedWeekOffset === 0);
  elements.lastWeekButton.classList.toggle("active", selectedWeekOffset === -1);
  elements.weekRange.textContent = formatWeekRange(weekStart);

  if (shifts.length === 0) {
    elements.weeklyDays.innerHTML =
      '<p class="weekly-empty">No hours recorded for this week.</p>';
    elements.weeklyTotal.textContent = "0h 0m";
    return;
  }

  let weeklyRoundedMinutes = 0;

  elements.weeklyDays.innerHTML = shifts.map((shift) => {
    const shiftMinutes = roundedTotalMinutes(shift.totalMilliseconds);
    weeklyRoundedMinutes += shiftMinutes;

    const dateLabel = new Date(shift.clockInTime).toLocaleDateString([], {
      weekday: "short",
      month: "short",
      day: "numeric"
    });

    const clockIn = formatTime(shift.clockInTime);
    const clockOut = shift.active ? "In progress" : formatTime(shift.clockOutTime);

    return `
      <div class="shift-row">
        <div class="shift-date">${dateLabel}</div>
        <div class="shift-details">
          <span>${clockIn} – ${clockOut}</span>
          <strong>${formatMinutes(shiftMinutes)}</strong>
        </div>
      </div>
    `;
  }).join("");

  elements.weeklyTotal.textContent = formatMinutes(weeklyRoundedMinutes);
}
...
elements.thisWeekButton.addEventListener("click", () => {
  selectedWeekOffset = 0;
  renderWeeklyHours();
});

elements.lastWeekButton.addEventListener("click", () => {
