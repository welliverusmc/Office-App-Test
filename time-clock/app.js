...
function roundToNearestMinute(milliseconds) {
  const totalWholeMinutes = Math.floor(milliseconds / 60000);
  const remainingMilliseconds = milliseconds % 60000;
  return totalWholeMinutes + (remainingMilliseconds > 30000 ? 1 : 0);
}

function formatHoursMinutes(milliseconds) {
  const totalMinutes = Math.max(0, roundToNearestMinute(milliseconds));
...
function renderWeeklyHours() {
  ...
  tickets.forEach((ticket) => {
    ...
    const totalMilliseconds = Number(ticket.totalMilliseconds);

    if (
      !Number.isFinite(clockInTime) ||
    ...
    const dayDate = new Date(clockInTime);
    dayDate.setHours(0, 0, 0, 0);
    const dayKey = dayDate.getTime();

    dailyTotals.set(
      dayKey,
      (dailyTotals.get(dayKey) || 0) + totalMilliseconds
    );
  });
  ...
  if (
    ...
    const dayDate = new Date(state.activeShift.clockInTime);
    dayDate.setHours(0, 0, 0, 0);
    const dayKey = dayDate.getTime();
    ...
    dailyTotals.set(
      ...
      (dailyTotals.get(dayKey) || 0) +
        (Date.now() - state.activeShift.clockInTime)
    );
  }
  ...
  elements.weeklyDays.innerHTML = rows.map(([dayKey, milliseconds]) => {
    weeklyTotalMilliseconds += milliseconds;
  ...
  elements.weeklyTotal.textContent =
    formatHoursMinutes(weeklyTotalMilliseconds);
}
